    ...
    <div className="border-t border-gray-100 bg-white/80 backdrop-blur-sm">
      <div className="px-4 md:px-6 py-3 md:py-4">
        <div className="flex items-center space-x-2 md:space-x-3">
          ...
          <div ref={scrollContainerRef} className="flex-1 flex space-x-2 md:space-x-3 overflow-x-auto scrollbar-hide">
            ...
            <button
              ...
              className={`flex-shrink-0 px-4 py-2 md:px-6 md:py-3 rounded-xl md:rounded-2xl text-sm md:text-base font-medium transition-all duration-300 flex items-center space-x-1.5 md:space-x-2 ${
                categoriaAtiva === null
                  ? 'bg-[#e58840] text-[#400b0b] shadow-lg shadow-[#e58840]/25 border border-[#e58840]'
                  : 'bg-white text-[#400b0b] hover:bg-gray-50 border border-gray-200 hover:border-gray-300 shadow-sm hover:shadow-md'
              }`}
            >
              <svg className={`w-4 h-4 md:w-5 md:h-5 ${categoriaAtiva === null ? 'text-[#400b0b]' : 'text-[#400b0b]/70'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                {categoria.icone_url ? (
                  ...
                  <img
                    ...
                    className={`w-4 h-4 md:w-5 md:h-5 object-contain ${
                      categoriaAtiva === categoria.categoria_id
                        ? 'filter brightness-0' // Remove invert para manter cor natural
                        : 'opacity-80'
                    }`}
                    onError={(e) => {
                  ...
                  <svg className={`w-4 h-4 md:w-5 md:h-5 ${categoriaAtiva === categoria.categoria_id ? 'text-[#400b0b]' : 'text-[#400b0b]/70'}`}
                       fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={categoria.icone_svg} />
                  </svg>
                ) : (
                  // Ícone padrão
                  <IconePadrao className={`w-4 h-4 md:w-5 md:h-5 ${categoriaAtiva === categoria.categoria_id ? 'text-[#400b0b]' : 'text-[#400b0b]/70'}`} />
                )}
          ...
          >
            <svg className="w-4 h-4 md:w-5 md:h-5 text-[#400b0b] group-hover:text-[#400b0b]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
